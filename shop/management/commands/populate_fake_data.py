...
class Command(BaseCommand):
    ...
    def _create_products(self, fake, count, batch_size):
        ...
        if existing_count >= count:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  Products already exist ({existing_count}). Skipping..."
                )
            )
            return
    ...
    def _create_customers(self, fake, count, batch_size):
        ...
        if existing_count >= count:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  Customers already exist ({existing_count}). Skipping..."
                )
            )
            return
        ...
        # Now create customers for the new users
    ...
    def _create_orders(self, fake, count, batch_size):
        ...
        for i in range(count):
            ...
            if (i + 1) % batch_size == 0:
                ...
                # Create order items for the batch
                for order in orders:
